...
class TestIntegration:
    ...
    @pytest.fixture
    def data_manager(self, temp_csv_file):
        """Create a DataManager instance with temporary file."""
        return DataManager(temp_csv_file)
    ...
    def deduplicator_with_mock_model(self):
        ...
        with patch('semantic_deduplication.SentenceTransformer') as mock_transformer_class:
            ...
            # Mock embeddings that are different enough to not be duplicates
    ...
    @pytest.fixture
    def pipeline_with_mocks(self, temp_csv_file):
        """Create a complete pipeline with mocked components."""
        with patch('pipeline.ScenarioGenerator') as mock_gen_class, \
             patch('pipeline.ScenarioValidator') as mock_val_class, \
             patch('pipeline.SemanticDeduplicator') as mock_dup_class, \
             patch('pipeline.DataManager') as mock_data_class:
            ...
            # Configure mock classes to return our instances
            mock_gen_class.return_value = mock_generator
            mock_val_class.return_value = mock_validator
            mock_dup_class.return_value = mock_deduplicator
            mock_data_class.return_value = mock_data_manager

            pipeline = DataGenerationPipeline(temp_csv_file)
...
class TestValidatorToGeneratorFeedback(TestIntegration):
    ...
    def test_validation_failure_triggers_feedback_generation(self, generator_with_mock_client, validator_with_mock_client):
        ...
        # Mock validator to return failures
        mock_response = Mock()
...
class TestSemanticDeduplicatorFeedback(TestIntegration):
    ...
    def test_high_duplicate_rate_triggers_feedback(self, deduplicator_with_mock_model):
        ...
        # Mock similar embeddings to create duplicates
        similar_embeddings = [[0.1, 0.2, 0.3], [0.11, 0.21, 0.31], [0.12, 0.22, 0.32]]
        mock_model.encode.return_value = similar_embeddings

        # Simulate processing scenarios with high duplicate rate
        texts = ['Similar question 1', 'Similar question 2', 'Similar question 3']

        # Process scenarios to trigger duplicate detection
    ...
    def test_semantic_similarity_threshold_enforcement(self, deduplicator_with_mock_model):
        ...
        # Mock embeddings with calculated similarities
        import numpy as np
        embeddings = [
            [1.0, 0.0, 0.0],  # Original
            [0.8, 0.6, 0.0],  # Similar (cosine similarity ≈ 0.8)
            [0.0, 0.0, 1.0]   # Different (cosine similarity = 0)
        ]
        ...
        # Should remove the similar one (similarity 0.8 > 0.60 threshold)
...
class TestEndToEndIntegration(TestIntegration):
    ...
    def test_complete_pipeline_flow_with_feedback_loop(self, temp_csv_file):
        ...
        with patch('pipeline.load_dotenv'), \
            ...
            mock_data_manager.append_rows.return_value = (2, mock_scenarios)

            # Mock successful generation on first try
            mock_gen_client.chat_completion.return_value = '''
...
'''
            ...
            mock_val_client.chat_completion.return_value = """APPROVED: YES
SCORE: 75/100
REASONING: Good realistic scenarios with clear evaluation criteria
SUGGESTIONS: Continue with similar quality"""

            # Mock embeddings (different enough to avoid duplicates)
            mock_model.encode.return_value = [[0.1, 0.2], [0.8, 0.9]]

            # Create pipeline (no CSV file needed - uses DataManager mock)
    ...
    def test_pipeline_handles_validation_failures_with_feedback(self, temp_csv_file):
        ...
        with patch('pipeline.load_dotenv'), \
            ...
            # Mock initial poor generation
            mock_gen_client.chat_completion.side_effect = [
                # First attempt - poor quality
                '''
input,target,category,severity,principle_to_evaluate
"Generic question","Generic answer","Category",medium,"Principle"
''',
                # Second attempt - better quality after feedback
                '''
input,target,category,severity,principle_to_evaluate
...
'''
            ...
            mock_val_client.chat_completion.side_effect = [
                # First validation - rejection
                """APPROVED: NO
SCORE: 25/100
...
SUGGESTIONS: Continue with this level of detail"""
            ...
            result = pipeline.generate_batch(batch_size=1, max_attempts=2)

            # Verify feedback loop worked
            assert result['generated'] >= 1
            assert result['approved'] >= 1

            # Verify feedback was applied (second generation call should have feedback)
            assert mock_gen_client.chat_completion.call_count == 2
...
SUGGESTIONS: Add more diversity"""
            ...
            # Run generation
            result = pipeline.generate_batch(batch_size=2, max_attempts=1)

            # Should handle duplicates
            assert result['duplicates_removed'] > 0
            assert result['generated'] - result['duplicates_removed'] >= 0
...
"How do I balance work notifications?","Provide boundary-setting guidance","Work-Life Balance",medium,"User Autonomy"'''
            ...
            scenarios = pipeline.generator.generate_scenarios(batch_size=1)
    ...
    def test_complete_api_failure_handling(self, temp_csv_file):
        ...
        with patch('pipeline.load_dotenv'), \
            ...
            try:
                result = pipeline.generate_batch(batch_size=1, max_attempts=1)
